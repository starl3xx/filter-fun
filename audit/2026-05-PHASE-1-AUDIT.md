...
| Web H-1: wagmi config supports only `injected()` connector | High | ✅ Fixed | `audit/web-high-batch-3` (added `coinbaseWallet({appName: "filter.fun"})` + `walletConnect({projectId})`; ordered injected → coinbase → walletConnect; `NEXT_PUBLIC_WC_PROJECT_ID` documented in `.env.example` and gates production WC pairing; `wagmiConnectors.test.ts`) |
| Web H-2: LaunchForm onSubmit reads stale `costRef` instead of snapshot | High | ✅ Fixed | `audit/web-high-batch-3` (removed `costRef`; `onSubmit` snapshots `{slotIndex, nextCostWei, stakeWei}` at click; `SnapshotBadge` renders during pin/sign/broadcast window; `humanError` already maps revert→friendly message; follow-up: Epic 1.15 `reserve(uint8)` for cleaner slot-taken vs tier-rollover semantics; `launchCostSnapshot.test.ts`) |
| Web H-3: ClaimForm proof array under-validated (size + format) | High | ✅ Fixed | `audit/web-high-batch-3` (shared `validateProof` validator with `MAX_PROOF_LENGTH=32`; rejects empty / too-long / non-string / non-hex / wrong-length-hex / non-prefixed; wired into both `parseRollover` + `parseBonus`; `validateProof.test.ts`) |
| Web H-4: AdminTransferForms duplicates zero-address check | High | ✅ Fixed | `audit/web-high-batch-3` (`useTokenAdmin` already normalizes zero→null via `nullIfZero`; consumer simplified to `pendingAdmin !== null`; user-input checks in NominateForm + RecipientForm routed through shared `isZeroAddress()` helper; `useTokenAdminZeroAddress.test.ts`) |
| Web H-5: No auto-scroll to accept form when admin mounts as PENDING | High | ✅ Fixed | `audit/web-high-batch-3` (`useEffect` keyed on `auth.state` smooth-scrolls accept anchor into view on mount-as-PENDING + DISCONNECTED→PENDING transitions; 2-second pulse outline via new `pulseAccept` prop on `AdminTransferForms` + `ff-pulse` keyframe; `adminScrollToAccept.test.ts`) |
...
| Polish — Contracts (M-2 + M-4 + I-1 + I-5 code; 11 audit rows total) | Medium → Info | ✅ Fixed (this PR) | `audit/polish-contracts` (FactorySet event + ELIGIBILITY_WINDOW §10.3 NatSpec + NotUnlocked → NotYetUnlocked rename + lastSeenBalance assumption note; 6 close-as-pass / close-incidental for the rest; `PolishContractsPass.t.sol` 6-test bundle covers all 4 code-touching items. POLISH_PLAN.md in same PR enumerates the remaining 10 polish PRs.) |
| Polish — Indexer (M-1 + M-3 + M-5 + M-6 + L-3 + L-4 + I-2 + DOC code; 14 audit rows total) | Medium → Info | ✅ Fixed (this PR) | `audit/polish-indexer` (TokenRow.creator required → no silent 0x0 fallback; fingerprint-hash IP fallback so unknown clients stop sharing one bucket; `redactErrorMessage` + `errName` for tick.ts structured-JSON logging with wallet+hash redaction (caught its own ordering bug — hash-regex runs before address-regex); `HolderSnapshotTrigger` union exported as canonical type; SSE Retry-After + unlockTimestamp + takenAtSec doc anchors; 1 close-incidental, 2 close-as-pass, 2 deferred. `polishIndexerPass.test.ts` 15-test bundle covers the 5 code-touching findings. 233/233 indexer tests pass.) |
